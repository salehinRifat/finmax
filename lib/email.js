...
// CLIENT_URL may be a comma-separated list (used for CORS); for email links
// we only want the first/canonical URL so we don't build broken hrefs.
...
// Logo lives in backend/assets so it ships with the backend on deploy
// (frontend isn't always on the same machine in production).
...
// OTP code rendered as a large monospace block. Used for both 2FA and
// password-reset flows; the surrounding copy differs by purpose.
function otpCard(code) {
  return `
    <div style="margin:20px 0;padding:18px 24px;background:#F5F4F0;border:1px solid #d0dde8;text-align:center;">
      <div style="font-family:Menlo,Consolas,monospace;font-size:30px;letter-spacing:8px;font-weight:600;color:#0a1a2a;">${code}</div>
    </div>`
}

export function sendLoginCodeEmail({ to, fullName, code }) {
  const greeting = fullName ? `Hello ${fullName},` : 'Hello,'
  const body = `
    <h1 style="font-family:Georgia,serif;font-size:22px;font-weight:600;color:#0a1a2a;margin:0 0 16px;">Your sign-in verification code</h1>
    <p style="font-size:14px;line-height:1.6;color:#244d73;margin:0 0 12px;">${greeting}</p>
    <p style="font-size:14px;line-height:1.6;color:#244d73;margin:0 0 12px;">
      Use the code below to finish signing in to your Finmax Services portal. The code expires in 10 minutes.
    </p>
    ${otpCard(code)}
    <p style="font-size:13px;line-height:1.6;color:#7099ba;margin:16px 0 0;">
      If you did not try to sign in, you can ignore this email — your account is still secure.
    </p>`
  return sendMail({
    to,
    subject: `Your Finmax sign-in code: ${code}`,
    html: WRAPPER(body),
    text: `${greeting}\n\nYour Finmax sign-in code is ${code}. It expires in 10 minutes.\nIf you didn't try to sign in, ignore this email.`,
  })
}

export function sendPasswordResetEmail({ to, fullName, code }) {
  const greeting = fullName ? `Hello ${fullName},` : 'Hello,'
  const body = `
    <h1 style="font-family:Georgia,serif;font-size:22px;font-weight:600;color:#0a1a2a;margin:0 0 16px;">Reset your password</h1>
    <p style="font-size:14px;line-height:1.6;color:#244d73;margin:0 0 12px;">${greeting}</p>
    <p style="font-size:14px;line-height:1.6;color:#244d73;margin:0 0 12px;">
      We received a request to reset your Finmax Services portal password. Enter the code below on the reset page to choose a new password. The code expires in 15 minutes.
    </p>
    ${otpCard(code)}
    <p style="font-size:13px;line-height:1.6;color:#7099ba;margin:16px 0 0;">
      If you didn't request a password reset, you can safely ignore this email — your current password will keep working.
    </p>`
  return sendMail({
    to,
    subject: `Your Finmax password reset code: ${code}`,
    html: WRAPPER(body),
    text: `${greeting}\n\nYour Finmax password reset code is ${code}. It expires in 15 minutes.\nIf you didn't request this, ignore this email.`,
  })
}
