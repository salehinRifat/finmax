  ...
  for (const form of BUILTIN_FORMS) {
    ...
    for (let i = 0; i < form.fields.length; i++) {
      ...
      const fieldExists = await queryOne(
        `SELECT id, is_builtin FROM form_fields WHERE form_key = ? AND field_key = ?`,
      ...
      } else if (!fieldExists.is_builtin) {
        // Older seed runs inserted these rows before the is_builtin column
        // existed (defaulted to 0). Re-stamp them so the "Additional
        // Questions" filter on the profile / PDF correctly hides them.
        await pool.execute(
          `UPDATE form_fields SET is_builtin = 1 WHERE id = ?`,
          [fieldExists.id]
        )
      }
    }
